...
**Reason (from Snowpark documentation):** N if parameter downcast is set. limit parameter only supported if method parameter is used.
...
The SMA adds the EWI `PNDSPY1051` on the output code to let you know that this element has a few scenarios not supported in Snowpark.
