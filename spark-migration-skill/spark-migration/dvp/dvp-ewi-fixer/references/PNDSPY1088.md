...
**Reason (from Snowpark documentation):** Only DatetimeIndex is supported and its freq will be lost. rule frequencies 's', 'min', 'h', and 'D' are supported. rule frequencies 'W', 'ME', and 'YE' are supported with closed = "left"
...
The SMA adds the EWI `PNDSPY1088` on the output code to let you know that this element has a few scenarios not supported in Snowpark.
