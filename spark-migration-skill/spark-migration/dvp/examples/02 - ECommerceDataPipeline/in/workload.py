...
from pyspark.sql.functions import col, sum, count, lit, rank, window, coalesce, abs, round, when
...
def run_pipeline(spark: SparkSession, dfs: dict[str, DataFrame]) -> dict[str, DataFrame]:
    ...
    foreign_txns_enriched = dfs["raw_transactions"].join(
        dfs["exchange_rates"].filter(col("currency_code") == "EUR").select(col("rate_date").alias("exchange_date"), "exchange_rate"),
        col("transaction_date") == col("exchange_date"), # Join on date to get the correct rate
        how="inner"
    ).withColumn(
...
def cleanup_existing_table_directory(spark: SparkSession, table_name: str) -> None:
...
def write_outputs(spark: SparkSession, output_dfs: dict[str, DataFrame]) -> None:
...
def show_outputs(spark: SparkSession, output_dfs: dict[str, DataFrame]) -> None:
    """Shows the contents of the output tables."""
    
    for table_name in output_dfs.keys():
        show_parquet_file(spark, table_name)

def show_parquet_file(spark: SparkSession, table_name):
...
def show_outputs_using_sql(spark: SparkSession, output_dfs: list[str]) -> None:
    ...
    spark.sql("""
        SELECT 
            ...
            format_number(transaction_amount_usd, 4) as transaction_amount_usd 
        FROM foreign_transactions"""
        ).show()

def read_existing_tables(spark: SparkSession) -> dict[str, DataFrame]:
    ...
    for table_name in table_names:
        ...
        if table_dir and os.path.exists(os.path.join(table_dir, "_SUCCESS")):
            try:
                ...
                try:
                    spark.sql(f"CREATE TABLE IF NOT EXISTS {table_name} USING PARQUET LOCATION '{table_dir}'")
                    spark.sql(f"SELECT * FROM {table_name}").show()
                except Exception:
                    pass
