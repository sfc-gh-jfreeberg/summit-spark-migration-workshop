"""Source test for 03_aggregations_pivot_windows_scos.ipynb entrypoint."""
import importlib
...
import pytest
# This notebook was already migrated to SCOS before the conversion — it requires
# snowpark_connect for session init, which is not available locally.
@pytest.mark.skip(reason="requires snowpark_connect which is not available in local PySpark source test environment")
class Test03AggregationsPivotWindowsScos(BaseSourceWorkloadTest):
    """Test suite for the 03_aggregations_pivot_windows_scos.ipynb source workload."""
    ...
    def _call_main(self, session):
        ...
        source_file=source_dir/"03_aggregations_pivot_windows_scos.ipynb.py"
        spec=importlib.util.spec_from_file_location("03_aggregations_pivot_windows_scos_ipynb_py",source_file)
