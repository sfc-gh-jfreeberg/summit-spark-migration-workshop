...
This issue appears when the SMA detects the use of a `Spark element of the RDD for Scala` that is not supported in `Snowpark Connect`, and does not have its own error code associated with it. This is the generic error code used by the SMA for an unsupported element.
...
val data = sc.parallelize(Seq(1, 2, 3, 4, 5), 3)
val coalesced = data.NotSupportedElement(1)
```

**Output**

The SMA adds the EWI `SPRKCNTSCL1500` to the output code to let you know that this element is not supported by Snowpark Connect.
...
val sc = new SparkContext(conf)
val data = sc.parallelize(Seq(1, 2, 3, 4, 5), 3)
// EWI SPRKCNTSCL1500: The element 'NotSupportedElement' of the library RDD is not supported for Snowpark Connect
val coalesced = data.NotSupportedElement(1)
```
