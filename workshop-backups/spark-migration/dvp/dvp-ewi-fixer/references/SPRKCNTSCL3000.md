...
This issue appears when the SMA detects the use of a `Spark element of MLLIB library for Scala` that is not supported in `Snowpark Connect`, and does not have its own error code associated with it. This is the generic error code used by the SMA for an unsupported element.
...
The SMA adds the EWI `SPRKCNTSCL3000` to the output code to let you know that this element is not supported by Snowpark Connect.
