...
pandas.core.frame.DataFrame.interpolate
...
Snowpark pandas currently has limitations with `DataFrame.interpolate`. It is not supported if axis == 1, limit is set, limit_area is "outside", or method is not "linear", "bfill", "backfill", "ffill", or "pad". And limit_area="inside" is supported only when method is linear.

## Scenarios

**Input**

The example below illustrates the not supported use of `pandas.core.frame.DataFrame.interpolate`.
...
The SMA adds the EWI `PNDSPY1015` to the output code to indicate that has a scenario not supported in Snowpark Pandas.
