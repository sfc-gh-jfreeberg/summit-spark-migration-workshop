...
**Reason (from Snowpark documentation):** Raises division by zero exception when right hand hand side contains at least one zero. pandas allows division by zero for non-object type Series and returns +/-inf.
...
The SMA adds the EWI `PNDSPY1213` on the output code to let you know that this element has a few scenarios not supported in Snowpark.
