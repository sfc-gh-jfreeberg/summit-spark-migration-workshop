...
**Reason (from Snowpark documentation):** The kind parameter has no effect. Snowpark pandas always uses a stable sort algorithm, while pandas by default does not.
...
The SMA adds the EWI `PNDSPY1209` on the output code to let you know that this element has a few scenarios not supported in Snowpark.
